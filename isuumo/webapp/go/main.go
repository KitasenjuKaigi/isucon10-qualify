...
func searchEstateNazotte(c echo.Context) error {
	...
	query := `SELECT * FROM estate FORCE INDEX(idx_latitude_longitude_popularity) WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? ORDER BY popularity DESC, id ASC`
